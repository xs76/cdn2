	...
	function k(e, t) {
		if ("symbolsUrl" != e) {
			...
			for (e in c.downloadProgress) {
				...
				var actualSize = 0;
				if (r && !r.total) {
					const componentURL = c[e]

					const o = componentURL.split("Build/")[1];
					actualSize = window.config.cachedDecompressedFileSizes ? window.config.cachedDecompressedFileSizes[o] : 0;

					if( navigator.userAgent.toLowerCase().indexOf('firefox') > -1 ){
						var avgCompression = 0.52;
						actualSize *= avgCompression;
					}
				}

				a++,
					r.lengthComputable	? ((n += r.loaded), (o += (r.total || actualSize)), s++)
						: r.finished || i++;
			}
			d(1.0 - a ? (a - i - (o ? (s * (o - n)) / o : 0)) / a : 0);
		}
	}
	function x() {
		var o = this;
							...
							return void 0 === e
								...
								: e.read().then(function (e) {
										if (e.done) {
											...
											for (
												var t = new Uint8Array(l), r = (t.set(i, 0), u), n = 0;
												n < d.length;
												++n
											)
												t.set(d[n], r), (r += d[n].length);
											return t;
										}
										return (
											l + e.value.length <= i.length
												? (i.set(e.value, l), (u = l + e.value.length))
												: d.push(e.value),
											(l += e.value.length),
											c({
												type: "progress",
